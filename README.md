...
温度是比较重要的指标，因此额外通过三色灯显示温度是否舒适，采用一般温度计的显示方法：蓝色表示冷，绿色表示合适，红色表示较热。

配色灵感来源见下图。以 `18.0` 和 `25.0` 作为分界。低于 `18.0`，认为是寒冷；高于 `25.0` 认为是炎；二者之间是舒适温度。不再考虑更加极端的情形。

突然发现有连个三色灯，那么对湿度也可以通过颜色进行可视化展示。标准仍然来源于现实中的温湿度计。 40% - 70% 为舒适，绿色；高于 70% 为蓝色；低于 40% 是红色。
...
这里注意，一定要把数码管的选择放到时序逻辑里，不然会显示错位。也就是，最高位显示在最低位，剩下的左移（详见 module sel_display7 的注释）。
