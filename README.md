...
温度的显示采用小数方式。这里有个问题，如果是零下，怎么办呢？如 -10.2，好像就无法正常显示了，因为位数不够。因此给出如下设定：当温度低于 `-9.9` 度时，显示 `Lo`。幸运的是，正数不用考虑本问题。
...
调试阶段，每 10s 采集一次数据，不必发送到服务器，而是在七段数码管上显示出来；应用阶段，每一分钟采集一次。
...
1. 下载[支持](https://dl.espressif.com/dl/package_esp32_index.json)，把链接填写到`File`->`Preferences`->`Additional boards manager URLs`中。下载的内容很大，而且国内直连比较困难。要么找镜像；要么不心疼流量，开全局+`TUN`。一定要两个都开喔！
...
```json
{
    "temperature": 25.6,    // 当前温度         2Byte
    "humidity": 58.9,       // 当前湿度         2Byte
    "ideal_tmp": 26.0,      // 理想温度         2Byte
    "diff": 3.5             // 温度的偏差值     1Byte
}
```
...
```json
{
    "time_type": "1h" | "1d" | "1w" | "1m", // 分别表示最近一小时，最近一天，最近一周，最近一个月。暂时没想提供那么丰富的查询功能
}
```
...
```json
    "data": [], // 存储数据的数组，数组的格式为温度，湿度。按时间升序排列，所以其实只需要排列好就可以
    "ideal_tmp": 26.5, // 理想温度
    "diff": 5.2, // 温度的偏差值
...
```json
{
    ...
    "timestamp",    // 时间戳，数据库自动生成，时间并不是硬件采集到温湿度的时间，而是接收到 packet 的时间，可能有点时间偏差，但是可以接受
}
```

## 未来展望

1. 传输数据更安全，网页访问的 `HTTPS` 应该已经可以安排上了，但是内容加密怎么办呢，后端可能还是先用 `HTTP` 吧？
2. 红外遥控
3. 自启动
4. 登录查看？那是计科导的事儿
